...
var Server = {
    ...
    create: function(requestedConfig) {
        ...
        if (typeof apiConfig.host !== "string") {
            if (typeof apiHost === "string") {
                deprecated(
                    "Please use apiConfig.host instead of apiHost",
                    "https://github.com/bloomberg/brisket/blob/master/docs/brisket.createserver.md#apiconfighost"
                );

                apiConfig.host = apiHost;
            } else {
                throw new Error(
                    "You cannot create a server without passing an apiConfig.host to createServer."
                );
            }
        }
        ...
        brisketEngine.use("/api", forwardClientRequestToApi(apiConfig));
        brisketEngine.get(
            "*",
