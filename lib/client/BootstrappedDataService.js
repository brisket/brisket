...
BootstrappedDataService.prototype = {
    ...
    checkAlreadyHasData: function(method, model, options) {
        if (!($.mockjax && this.bootstrappedData)) {
            return;
        }
        ...
        var mockjaxIdForUrl = this.mockjaxIds[key];

        if (mockjaxIdForUrl !== undefined) {
            $.mockjax.clear(mockjaxIdForUrl);
            return;
        }

        mockjaxIdForUrl = $.mockjax({
            url: url,
            data: options.data,
        ...
        this.mockjaxIds[key] = mockjaxIdForUrl;
    },

    clearMockedData: function() {
        for (var id in this.mockjaxIds) {
            $.mockjax.clear(this.mockjaxIds[id]);
        }
