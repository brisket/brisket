...
var ClientRenderingWorkflow = {
    ...
    createHandlerFrom: function(originalHandler) {
        return function() {
            ...
            function finishRoute(requestId) {
                /* [wawjr3d][4/11/2016] enter dom if there is an error in the route.
                 *  Do it in the finishRoute to minimize error output from promise
                 *  chain and funnel it to Errors.notify
                 */
                inCaseOfErrorEnterDOM(layout);
...
function enterDOM(layout) {
    errorCameFromEnteringDOM = true;
    layout.enterDOM();
    errorCameFromEnteringDOM = false;
}

function inCaseOfErrorEnterDOM(layout) {
    if (errorCameFromEnteringDOM) {
        return;
    }
...
function redirectOrLogAndRenderError(forRoute) {
    return function(reason) {
        ...
        var clientRequest = forRoute.clientRequest;
        var requestId = forRoute.requestId;
        var renderLayout = forRoute.renderLayout;
        ...
        return render(layout, errorView, requestId);
    };
}
